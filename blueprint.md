...
3.  **執行步驟**：
    *   **任務 1**: 設定 Vuetify 主題與全域樣式。
    *   **任務 2**: 建立三欄式佈局與導覽列。
    *   **任務 3**: 建立左側分類選單元件 (`CategoryMenu.vue`)。
    *   **任務 4**: 建立商品卡片元件 (`ProductCard.vue`)。
    *   **任務 5**: 建立右側購物車元件 (`ShoppingCart.vue`)。
    *   **任務 6**: 組合元件並完成主頁面 (`index.vue`)。
    *   **任務 7**: 實現 RWD 規則。
    *   **任務 8**: 更新所有相關 `README.md` 檔案。
4.  **驗證計畫**：
